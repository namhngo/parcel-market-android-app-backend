...
namespace Gis.API.Controllers
{
    ...
    public class Por_LogGisController : ApiControllerBase<Por_LogSearchGis>
    {
        private readonly IUploadFileProvider _fileProvider;
        private readonly IServiceWrapper _service;
        private readonly ILogger<Por_LogGisController> _logger;
        private readonly string _savePath;
        public Por_LogGisController(IConfiguration rootConfiguration, IUploadFileProvider fileProvider, IServiceWrapper service, ILogger<Por_LogGisController> logger) : base(service, logger)
        {
        ...
        public async Task<IActionResult> SearchDate([FromBody] TimkiemLogGis gis)
        {
            try
            {
                var items = await _service.Por_LogSearch.SearchDate(gis.toDate, gis.endDate);
