...
{
    public class Service:RepositoryBase<Model.Por_GopYPhanAnh>, Por_HoSoNguoiNopPA.IService
    {
        ...
        private async Task SendMessageHoSoPhanAnh(Guid IDHoSo, string TypeSms, string TypeEmail)
        {
            try
            {
                ...
                var item = await (from x in _dbContext.Por_GopYPhanAnhs
                                  ...
                                  join a in _dbContext.Sys_Categories on z.IDLoaiPhanAnh equals a.Id
                                       into sub_Sys_Categories
                                  from aa in sub_Sys_Categories.DefaultIfEmpty()
        ...
        private async Task SendMessageHoSoPhanAnhTiepNhan(Guid IDHoSo, Guid IDBuocQuyTrinh, string TypeSms, string TypeEmail)
        {
            try
            {
                string template_send_message_insert = "INSERT INTO public.por_smssend (mobile, email, content, contentemail, status, statusmail, time_create, time_send, type) VALUES";
        ...
        private async Task SendMessageHoSoPhanAnhTraNguocLai(Guid IDHoSo, Guid IDQuyTrinh, Guid IDBuocQuyTrinh, string TypeSms, string TypeEmail)
        {
            try
            {
                string template_send_message_insert = "INSERT INTO public.por_smssend (mobile, email, content, contentemail, status, statusmail, time_create, time_send, type) VALUES";
                ...
                var TinhThanhPho = await _dbContext.Por_TinhThanhPhos.Where(o => o.Id == item.TinhThanhPho).FirstOrDefaultAsync();
                #region BuocQuyTrinh
                var buocquytrinhPrev = await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.FirstOrDefaultAsync(o => o.IDHoSo == IDHoSo && o.IDQuyTrinh == IDQuyTrinh && o.IDBuocQuyTrinh == IDBuocQuyTrinh);
        ...
        private async Task SendMessageHoSoPhanAnh(Guid IDHoSo, string[] SendUsers, string NguoiXuLy, bool guiSms, Guid? idMauSms, bool guiEmail, Guid? idMauEmail)
        {
            try
            {
                if (!guiSms && !guiEmail)
        ...
        public async Task<List<TraCuuHoSoPA>> TraCuuHoSo(string MaPhanAnh, string UserName)
        {
            var query = (from x in _dbContext.Por_GopYPhanAnhs
                         ...
                         select new TraCuuHoSoPA()
                         {
                             ...
                             IdThuTuc = o.IDQuyTrinh
                         });
            if (!string.IsNullOrEmpty(MaPhanAnh))
            {
                query = query.Where(o => o.MaPhanAnh.ToLower().Contains(MaPhanAnh.ToLower()));
            }
            ...
            return await query.OrderByDescending(o => o.NgayGuiDate).ToListAsync();
        }
        public async Task<bool> KiemTraBuocCuoiCung(Guid Id)
        {    
            var hoso_quytrinh = await _dbContext.Por_HoSoPA_QuyTrinhs.Where(o => o.IDHoSo == Id).FirstAsync();
        ...
        public async Task<Guid> ChuyenXuLy(Guid Id, string NoiDung,string UserName)
        {
            ...
            var DsBuocQuyTrinh = await (from x in _dbContext.Por_BuocQuyTrinhs
                                        ...
                                        join z in _dbContext.Por_HoSoPA_QuyTrinhs on x.IDQuyTrinh equals z.IDQuyTrinh
            ...
            var hosoquytrinh = await _dbContext.Por_HoSoPA_QuyTrinhs.Where(o => o.IDHoSo == Id).FirstAsync();
            ...
            var hoso = await _dbContext.Por_GopYPhanAnhs.Where(o => o.Id == Id).FirstAsync();
            ...
            hoso.IDTrangThaiPA = trangthai_choxuly.Id;
            //
            var HoSoBuocQuyTrinhCurrent = await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.Where(o => o.IDHoSo == hosoquytrinh.IDHoSo && o.IDQuyTrinh == hosoquytrinh.IDQuyTrinh && o.IDBuocQuyTrinh == id_buocquytrinhcurrent).FirstAsync();
            ...
            if (id_buocquytrinhnext == Guid.Empty)//
            {
                ...
                hoso.IDTrangThaiPA = trangthai_traketqua.Id;
                await SendMessageHoSoPhanAnh(Id, SmsConfig.Mau_Sms_HSPA_TraKetQua.ToString(), EmailConfig.Mau_Email_HSPA_TraKetQua.ToString());
            }    
            else
            {                                
                hosoquytrinh.IDBuocQuyTrinh = id_buocquytrinhnext;
                ...
                if (laBuocCuoiCung)
                {
                    await SendMessageHoSoPhanAnh(Id, SmsConfig.Mau_Sms_HSPA_ToiBuocCuoi.ToString(), EmailConfig.Mau_Email_HSPA_ToiBuocCuoi.ToString());
                }
                await SendMessageHoSoPhanAnh(Id, dsNguoiDungThamGiaSplit, hosoquytrinh.NguoiXuLy, guiSms, idMauSms, guiEmail, idMauEmail);
            }
        ...
        public async Task HuyQuyTrinh(Guid Id, string UserName, string NoiDungHuy)
        {
            ...
            HoSo.IDTrangThaiPA = trangthai_huy.Id;
            var HoSoQuyTrinh = await _dbContext.Por_HoSoPA_QuyTrinhs.Where(o => o.IDHoSo == Id).FirstAsync();
            HoSoQuyTrinh.NgayHuy = DateTimeOffsetNow;
            HoSoQuyTrinh.NguoiHuy = UserName;
            HoSoQuyTrinh.NoiDungHuy = NoiDungHuy;
            var HoSoBuocQuyTrinhCurrent = await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.Where(o => o.IDHoSo == HoSoQuyTrinh.IDHoSo && o.IDQuyTrinh == HoSoQuyTrinh.IDQuyTrinh && o.IDBuocQuyTrinh == HoSoQuyTrinh.IDBuocQuyTrinh).FirstOrDefaultAsync();
            if (HoSoBuocQuyTrinhCurrent != null)
            {
                HoSoBuocQuyTrinhCurrent.IDTrangThai = trangthai_huy.Id;
                HoSoBuocQuyTrinhCurrent.NguoiXuLy = UserName;
            }
            await SendMessageHoSoPhanAnh(Id, SmsConfig.Mau_Sms_HSPA_Huy.ToString(), EmailConfig.Mau_Email_HSPA_Huy.ToString());
            await UnitOfWork.SaveAsync();
        }
        public async Task<Guid> TraNguocLai(Guid Id, string NoiDung, string UserName)
        {
            ...
            var hosobuocquytrinh_prev = await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.Where(o => o.IDHoSo == Id && o.IDQuyTrinh == hosoquytrinh.IDQuyTrinh && o.IDBuocQuyTrinh == id_buocquytrinhprev).FirstAsync();
            ...
            //            
            var hosobuocquytrinh_current = await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.Where(o => o.IDHoSo == Id && o.IDQuyTrinh == hosoquytrinh.IDQuyTrinh && o.IDBuocQuyTrinh == id_buocquytrinhcurrent).FirstOrDefaultAsync();
            if(hosobuocquytrinh_current != null)
            {
                hosobuocquytrinh_prev.IDTrangThai = trangthai_choxuly.Id;
            }            
            //
            var hoso = _dbContext.Por_GopYPhanAnhs.Where(o => o.Id == Id).First();
            hoso.IDTrangThaiPA = trangthai_tranguoclai.Id;
            await SendMessageHoSoPhanAnhTraNguocLai(Id, hosoquytrinh.IDQuyTrinh, id_buocquytrinhprev, SmsConfig.Mau_Sms_HSPA_TraNguocLai.ToString(), EmailConfig.Mau_Email_HSPA_TraNguocLai.ToString());
            await UnitOfWork.SaveAsync();
        ...
        {
            var DateTimeOffsetNow = DateTimeOffset.Now.AddHours(Sys_Const.TimeZone);
            var HoSoQuyTrinh = await _dbContext.Por_HoSoPA_QuyTrinhs.FirstAsync(o => o.IDHoSo == Id);
            HoSoQuyTrinh.NgayNhanKQ = DateTimeOffsetNow;
            var HoSo = await _dbContext.Por_GopYPhanAnhs.FirstAsync(o => o.Id == Id);
            ...
            HoSo.IDTrangThaiPA = trangthai_traketqua.Id;
            var por_HoSo_Buoc_QuyTrinh = await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.Where(o => o.IDHoSo == Id && o.IDQuyTrinh == HoSoQuyTrinh.IDQuyTrinh && o.IDBuocQuyTrinh == HoSoQuyTrinh.IDBuocQuyTrinh).FirstAsync();
            por_HoSo_Buoc_QuyTrinh.NgayKetThuc = DateTimeOffset.Now;
        ...
        public async Task TiepNhanHoSo(Guid Id, string UserName)
        {
            ...
            if (HoSoQuyTrinh.NgayTiepNhan == null)
            {
                ...
                await SendMessageHoSoPhanAnhTiepNhan(Id, HoSoQuyTrinh.IDBuocQuyTrinh, SmsConfig.Mau_Sms_HSPA_TiepNhan.ToString(), EmailConfig.Mau_Email_HSPA_TiepNhan.ToString());
            }
            var HoSo = await _dbContext.Por_GopYPhanAnhs.FirstAsync(o => o.Id == Id);
            ...
            HoSo.IDTrangThaiPA = trangthai_dangxuly.Id;
            var HoSoBuocQuyTrinh = await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.FirstOrDefaultAsync(o => o.IDHoSo == HoSoQuyTrinh.IDHoSo && o.IDQuyTrinh == HoSoQuyTrinh.IDQuyTrinh && o.IDBuocQuyTrinh == HoSoQuyTrinh.IDBuocQuyTrinh);
            if (HoSoBuocQuyTrinh == null)
            {
                HoSoBuocQuyTrinh = new Model.Por_HoSoPA_Buoc_QuyTrinh();
                ...
                await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.AddAsync(HoSoBuocQuyTrinh);
            }
            else
            {
                HoSoBuocQuyTrinh.IDTrangThai = trangthai_dangxuly.Id;
        ...
        public async Task<List<DsBuocQuyTrinhHoSo>> DsBuocQuyTrinhHoSo(Guid Id)
        {
            ...
            {
                var trangthais = await _dbContext.Sys_Categories.Where(o => o.Type == Core.Enums.CategoryType.TrangThaiHoSo).ToListAsync();
                var hoso_buocquytrinh = await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.Where(o => o.IDHoSo == Id && o.IDQuyTrinh == dsbuocquytrinh[0].IdQuyTrinh).ToListAsync();
            ...
            var current_buocquytrinh = await _dbContext.Por_HoSoPA_QuyTrinhs.FirstAsync(o => o.IDHoSo == Id);
        ...
        public async Task<LuuHoSoPAQuyTrinh> LuuHoSoQuyTrinh(LuuHoSoPAQuyTrinh model)
        {            
            if (model.Id != Guid.Empty)
            {
                var entry = await _dbContext.Por_GopYPhanAnhs.FirstAsync();
                ObjectHelpers.Mapping<LuuHoSoPAQuyTrinh, Model.Por_GopYPhanAnh>(model, entry, new[] { "Id" });                 
            ...
            var modelSave = new Model.Por_GopYPhanAnh();
            ObjectHelpers.Mapping<LuuHoSoPAQuyTrinh, Model.Por_GopYPhanAnh>(model, modelSave);
            modelSave.Id = Guid.NewGuid();
            modelSave.MaPhanAnh = Guid.NewGuid().ToString();
            modelSave.IDTrangThaiPA = TrangThaiChoXuLy.Id;
            model.Id = modelSave.Id;
            await _dbContext.Por_GopYPhanAnhs.AddAsync(modelSave);            
            ...
            por_HoSo_QuyTrinh.NgayNop = DateTimeOffset.Now;
            por_HoSo_QuyTrinh.NgayTiepNhan = null;
            por_HoSo_QuyTrinh.NgayNhanKQ = null;
            por_HoSo_QuyTrinh.NgayDuKienNhanKQ = DateTimeOffset.Now.AddDays(BuocQuyTrinhFirst.ThoiGianThucHien);
            await _dbContext.Por_HoSoPA_QuyTrinhs.AddAsync(por_HoSo_QuyTrinh);
            await UnitOfWork.SaveAsync();
            return model;
        }
        public async Task<ChiTietHoSoPAQuyTrinh> LayChiTietHoSoQuyTrinh(Guid id)
        {
            var hoso = await _dbContext.Por_GopYPhanAnhs.FirstAsync(o => o.Id == id);
            var quytrinh = await _dbContext.Por_HoSoPA_QuyTrinhs.FirstAsync(o => o.IDHoSo == id);
            ChiTietHoSoPAQuyTrinh chiTietHoSo = new ChiTietHoSoPAQuyTrinh();
            ObjectHelpers.Mapping<Model.Por_GopYPhanAnh, ChiTietHoSoPAQuyTrinh>(hoso, chiTietHoSo);
        ...
        public async Task<List<HoSoPAHuy>> HoSoHuy(string MaPhanAnh, string UserName)
        {
            ...
            var query = from x in _dbContext.Por_GopYPhanAnhs
                        join y in _dbContext.Sys_Categories.Where(o => o.Type == Core.Enums.CategoryType.TrangThaiHoSo) on x.IDTrangThaiPA equals y.Id
                        join z in _dbContext.Por_HoSoPA_QuyTrinhs on x.Id equals z.IDHoSo
                        join j in _dbContext.Sys_Categories.Where(o => o.Type == Core.Enums.CategoryType.LoaiPhanAnh) on x.IDLinhVuc equals j.Id
                        where x.IDTrangThaiPA == trangthai.Id
                        select new { x.Id, IdThuTuc = z.IDQuyTrinh, LoaiPhanAnh = j.Name, x.MaPhanAnh, TrangThai = y.Name, z.NgayNop, z.NgayHuy, z.NguoiHuy, z.NoiDungHuy };
            if (!string.IsNullOrEmpty(MaPhanAnh))
            {
                query = query.Where(o => o.MaPhanAnh.ToLower().Contains(MaPhanAnh.ToLower()));
            }
            ...
            return await query.OrderByDescending(o => o.NgayHuy).Select(o => new HoSoPAHuy()
            {
                Id = o.Id,
                MaPhanAnh = o.MaPhanAnh,
                LoaiPhanAnh = o.LoaiPhanAnh,
        ...
        public async Task<List<HoSoPAHoanThanh>> HoSoHoanThanh(string MaPhanAnh)
        {
            ...
            var query = from x in _dbContext.Por_GopYPhanAnhs
                        ...
                        join j in _dbContext.Sys_Categories.Where(o => o.Type == Core.Enums.CategoryType.LoaiPhanAnh) on x.IDLinhVuc equals j.Id
                        //where x.IDTrangThaiPA == trangthai.Id
                        select new { x.Id, LoaiPhanAnh = j.Name, x.MaPhanAnh, TrangThai = y.Name, z.NgayNop };
            if (!string.IsNullOrEmpty(MaPhanAnh))
            {
                query = query.Where(o => o.MaPhanAnh.ToLower().Contains(MaPhanAnh.ToLower()));
            }
            return await query.Select(o => new HoSoPAHoanThanh()
            {
                Id = o.Id,
                MaPhanAnh = o.MaPhanAnh,
                LoaiPhanAnh = o.LoaiPhanAnh,
        ...
        public async Task<List<HoSoPAXuLy>> HoSoXuLy(Guid UserId, string UserName, string MaPhanAnh)
        {
            var trangthai = await _dbContext.Sys_Categories.Where(o => o.Type == Core.Enums.CategoryType.TrangThaiHoSo).ToListAsync();
            var trangthai_choxuly = trangthai.Where(o => o.Code == TrangThaiQuyTrinh.choxuly.ToString()).First();
            var trangthai_dangxuly = trangthai.Where(o => o.Code == TrangThaiQuyTrinh.dangxuly.ToString()).First();            
            ...
            var query = from x in _dbContext.Por_GopYPhanAnhs
                        ...
                        join j in _dbContext.Sys_Categories.Where(o => o.Type == Core.Enums.CategoryType.LoaiPhanAnh) on o.IDLoaiPhanAnh equals j.Id
                        join a in _dbContext.Por_PhuongXaThiTrans on x.PhuongXa equals a.Id
                        where
                        (x.IDTrangThaiPA == trangthai_choxuly.Id
                        || x.IDTrangThaiPA == trangthai_dangxuly.Id                        
                        || x.IDTrangThaiPA == trangthai_tranguoclai.Id) 
                        && i.IDsNguoiDungThamGia.ToLower().Contains(UserId.ToString().ToLower())
                        select new { x.Id, IdThuTuc = o.Id, TenPhuongXa = a.Ten, x.TieuDe, x.SoNha, x.TenDuong, x.MaPhanAnh, LoaiPhanAnh = j.Name, TrangThai = y.Name, x.TenNguoiGui, x.Email, x.SoDienThoai, z.NgayNop, z.NgayTiepNhan, z.NguoiXuLy, z.IDQuyTrinh, IDBuocQuyTrinh = i.Id };               
            if (!string.IsNullOrEmpty(MaPhanAnh))
            {
                query = query.Where(o => o.MaPhanAnh.ToLower().Contains(MaPhanAnh.ToLower()));
            }
            ...
            //    query = query.Where(o => o.IdThuTuc == Guid.Empty);
            //}
            var items = await query.OrderByDescending(o => o.NgayNop).Select(o => new HoSoPAXuLy()
            {
                Id = o.Id,
                ...
                LoaiPhanAnh = o.LoaiPhanAnh,
                TrangThai = o.TrangThai,
                NguoiNop = o.TenNguoiGui,
                TieuDe = o.TieuDe,
                SoNha = o.SoNha,
                TenDuong = o.TenDuong,
                PhuongXa = o.TenPhuongXa,
                NgayNop = o.NgayNop.ToString("dd/MM/yyyy HH:mm:ss"),
            ...
            var items_filter = new List<HoSoPAXuLy>();            
        ...
        {
            var Por_GopYPhanAnhs = await _dbContext.Por_GopYPhanAnhs.FirstOrDefaultAsync(o => o.Id == Id);
            if(Por_GopYPhanAnhs != null)
            {
                _dbContext.Por_GopYPhanAnhs.Remove(Por_GopYPhanAnhs);
            } 
            ...
            //}
            var Por_HoSoPA_QuyTrinhs = await _dbContext.Por_HoSoPA_QuyTrinhs.FirstOrDefaultAsync(o => o.IDHoSo == Id);
            if(Por_HoSoPA_QuyTrinhs != null)
            {
                _dbContext.Por_HoSoPA_QuyTrinhs.Remove(Por_HoSoPA_QuyTrinhs);
            }               
            var Por_HoSoPA_Buoc_QuyTrinhs = await _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.FirstOrDefaultAsync(o => o.IDHoSo == Id);
            if(Por_HoSoPA_Buoc_QuyTrinhs != null)
            {
                _dbContext.Por_HoSoPA_Buoc_QuyTrinhs.Remove(Por_HoSoPA_Buoc_QuyTrinhs);
            }
            await _dbContext.SaveChangesAsync();
        }    
        public async Task<List<HoSoPATamDung>> HoSoTamDung(string MaPhanAnh)
        {
            ...
            var query = from x in _dbContext.Por_GopYPhanAnhs
                        ...
                        join z in _dbContext.Por_HoSoPA_QuyTrinhs on x.Id equals z.IDHoSo
                        where x.IDTrangThaiPA == trangthai.Id
                        select new { x.Id, x.MaPhanAnh, TrangThai = y.Name, z.NgayNop, z.NgayTamNgung };
            if (!string.IsNullOrEmpty(MaPhanAnh))
            {
                query = query.Where(o => o.MaPhanAnh.ToLower().Contains(MaPhanAnh.ToLower()));
            }
            return await query.Select(o => new HoSoPATamDung()
            {
                Id = o.Id,
                MaPhanAnh = o.MaPhanAnh,
                TrangThai = o.TrangThai,
                NgayNop = o.NgayNop.ToString("dd/MM/yyyy HH:mm:ss"),
                NgayTamNgung = (o.NgayTamNgung.HasValue ? o.NgayTamNgung.Value.ToString("dd/MM/yyyy HH:mm:ss") : "")
            }).ToListAsync();
        }
        public async Task<List<HoSoPATraKetQua>> HoSoTraKetQua(string MaPhanAnh, string UserName)
        {
            ...
            var query = from x in _dbContext.Por_GopYPhanAnhs
                        join y in _dbContext.Sys_Categories on x.IDTrangThaiPA equals y.Id
                        join z in _dbContext.Por_HoSoPA_QuyTrinhs on x.Id equals z.IDHoSo
                        join o in _dbContext.Por_QuyTrinhs on z.IDQuyTrinh equals o.Id
                        join j in _dbContext.Sys_Categories.Where(o => o.Type == Core.Enums.CategoryType.LoaiPhanAnh) on o.IDLoaiPhanAnh equals j.Id
                        where x.IDTrangThaiPA == trangthai.Id
                        select new { x.Id, IdThuTuc = o.Id, LoaiPhanAnh = j.Name, x.MaPhanAnh, TrangThai = y.Name, z.NgayNop, z.NgayNhanKQ, z.NguoiTraKetQua };
            if (!string.IsNullOrEmpty(MaPhanAnh))
            {
                query = query.Where(o => o.MaPhanAnh.ToLower().Contains(MaPhanAnh.ToLower()));
            }
            ...
            return await query.OrderByDescending(o => o.NgayNhanKQ).Select(o => new HoSoPATraKetQua()
            {
                Id = o.Id,
                MaPhanAnh = o.MaPhanAnh,
                LoaiPhanAnh = o.LoaiPhanAnh,
